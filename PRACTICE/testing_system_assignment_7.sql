...
DROP TRIGGER IF EXISTS trg_max5Account;
DELIMITER $$   
CREATE TRIGGER trg_max5Account
BEFORE INSERT ON `GroupAccount` 
FOR EACH ROW
BEGIN 
DECLARE v_amount_acc TINYINT DEFAULT 0;
SELECT count(*) INTO  v_amount_acc FROM `GroupAccount` WHERE GroupID =NEW.GroupID;
IF (v_amount_acc > 5) THEN
-- dừng chương trình
SIGNAL SQLSTATE '12345'
-- gửi thông báo
...
END $$ 
DELIMITER ;
INSERT INTO `GroupAccount` (`GroupID`, `AccountID`) VALUES ('1', '13');

-- Question 4: Cấu hình 1 bài thi có nhiều nhất là 10 Question 
DROP TRIGGER IF EXISTS trg_max5Question;
DELIMITER $$   
CREATE TRIGGER trg_max5Question
BEFORE INSERT ON ExamQuestion 
FOR EACH ROW
BEGIN   
DECLARE v_amountQuestion TINYINT DEFAULT 0;
SELECT count(*) INTO v_amountQuestion FROM ExamQuestion WHERE ExamID= NEW.ExamID;
IF (v_amountQuestion > 5) THEN
-- dừng chương trình
SIGNAL SQLSTATE '12345'
-- gửi thông báo
SET MESSAGE_TEXT = 'Cant add more Account';
...
END $$ 
DELIMITER ;
INSERT INTO ExamQuestion (`ExamID`, `QuestionID`) VALUES ('1', '15');
...
DROP TRIGGER IF EXISTS trg_DlAdmin;
DELIMITER $$   
CREATE TRIGGER trg_DlAdmin
...
DROP TRIGGER IF EXISTS trg_bfInsertAccount;
DELIMITER $$   
CREATE TRIGGER trg_bfInsertAccount
BEFORE INSERT ON  `Account` 
FOR EACH ROW
BEGIN 
	DECLARE newDep TINYINT DEFAULT 0;
	SELECT DepartmentID INTO newDep FROM Department WHERE DepartmentName LIKE 'Waiting Department';
	IF (NEW.DepartmentID IS NULL) THEN 
		-- SELECT DepartmentID FROM `Account` WHERE NEW.DepartmentID=newDep;
		SET NEW.DepartmentID = newDep; 
        END IF;
	IF (NEW.DepartmentID =0) THEN
		-- dừng chương trình
		SIGNAL SQLSTATE '12345'
		-- gửi thông báo
		SET MESSAGE_TEXT = 'Waiting Department doesnt exist';
	END IF;
END $$ 
DELIMITER ;
INSERT INTO `testing_system_assignment_4`.`Account` (`Email`, `Username`, `FullName`, `PositionID`) 
VALUES ('Thuy1234@gmail.com', 'Thuy1234', 'Nguyễn Hồng M', '2');
...
FOR EACH ROW
BEGIN 
	DECLARE v_createDateExam DATETIME;
    SET  v_createDateExam=date_sub(now(),INTERVAL 2 day);
	IF (OLD.CreateDate > v_createDateExam) THEN 
		-- dừng chương trình
		SIGNAL SQLSTATE '12345'
		-- gửi thông báo
...
DROP TRIGGER IF EXISTS trg_changeGender;
DELIMITER $$   
CREATE TRIGGER trg_changeGender
...
-- phân tích: 1) mỗi question có tối đa 4 câu trả lời, 2) mỗi câu hỏi (NEW.câu hỏi = OLD câu hỏi và new. câu trả lời = old đáp án)
...
DROP TRIGGER IF EXISTS trg_updateQues;
DELIMITER $$   
CREATE TRIGGER trg_updateQues
BEFORE UPDATE ON  Question 
FOR EACH ROW
BEGIN 
	DECLARE v_countQues TINYINT DEFAULT 0;
   SELECT count(*) INTO v_countQues FROM ExamQuestion WHERE QuestionID= NEW.QuestionID;
	IF (v_countQues > 0) THEN 
		-- dừng chương trình
		SIGNAL SQLSTATE '12345'
		-- gửi thông báo
		SET MESSAGE_TEXT = 'Can not update this record';
	END IF;
END $$ 
DELIMITER ;

UPDATE `Question` 
SET `Content` = 'Hỏi về C##' WHERE (`QuestionID` = '3') and (`CategoryID` = '9') and (`TypeID` = '2');
-- cách 2: sử dụng IF EXISTS (SELECT 1 FROM .... WHERE ...=...) THEN
DROP TRIGGER IF EXISTS trg_updateQues;
DELIMITER $$   
CREATE TRIGGER trg_updateQues
BEFORE UPDATE ON  Question 
FOR EACH ROW
BEGIN 
	IF EXISTS (SELECT 1 FROM ExamQuestion WHERE QuestionID= NEW.QuestionID) THEN 
		-- dừng chương trình
		SIGNAL SQLSTATE '12345'
		-- gửi thông báo
		SET MESSAGE_TEXT = 'Can not update this record';
...
END $$ 
DELIMITER ;
UPDATE Question SET Content ='Hỏi về C##' WHERE QuestionID=3;

-- Trigger để không cho xóa:
DROP TRIGGER IF EXISTS trg_beDelQues;
DELIMITER $$   
CREATE TRIGGER trg_beDelQues
BEFORE DELETE ON  Question
FOR EACH ROW
BEGIN 
	IF EXISTS (SELECT 1 FROM ExamQuestion WHERE QuestionID= OLD.QuestionID) THEN 
    SIGNAL SQLSTATE '12345'
    SET MESSAGE_TEXT ='Can delete this record';
	END IF;
END $$ 
DELIMITER ;
...
SELECT ga.GroupID, count(*) , 		CASE
									WHEN count(*) <= 5 THEN 'Few'
                                    WHEN count(*) <= 20 THEN 'Normal'
                                    ELSE 'Higher'
                                    END AS the_number_user_amount

FROM `GroupAccount` ga GROUP BY ga.GroupID;
...
SELECT d.DepartmentName,					CASE
											WHEN count(*)= 0 THEN 'Không có User'
                                            ELSE count(*)
											END AS the_number_user_amount
                                            FROM `Account` a 
